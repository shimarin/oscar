...
.controller("AdminController", ["$scope","$resource", function($scope, $resource) {
    var info_api = $resource("./info")
    var share_info_api = $resource("./:share_name/info")
    $scope.info = info_api.get();

    $scope.new_share_selected = function() {
        $scope.share = {new:true};
    }
    $scope.share_selected = function(share_name) {
        $scope.share = share_info_api.get({share_name:share_name});
    }

...
.controller("ShareController", ["$scope", "$resource","$location","$timeout", function($scope, $resource,$location,$timeout) {
    ...
    $scope.load = function(path) {
        ...
        $scope.info = info.get({path:$scope.path});
    ...
    function exec_search(path, q, limit) {
        ...
        s.result = search.get({path:path,q:q,limit:limit}, function() {
            s.ongoing = false;
        }, function() {
            s.ongoing = false;
        });
    ...
    $scope.$watchCollection("[path, q, limit]", function(newValues,oldValues) {
        ...
        else $scope.search = {ongoilg:false, result: null};
    });
    ...
    $scope.open_file = function(name) {
        window.open($scope.join_path('.',$scope.join_path($scope.path, name)));
    }
